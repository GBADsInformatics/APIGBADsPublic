from app.utils.auth import SlackJWTVerifier, CognitoVerifier
...
def test_when_valid_bearer_token_then_no_exception():
    verifier = CognitoVerifier(expected_token="goodtoken")
...
def test_when_invalid_bearer_token_then_raises_http_401():
    ...
    with pytest.raises(HTTPException) as excinfo:
        verifier(api_key)
    assert excinfo.value.status_code == 401
    assert "Invalid or missing authentication" in str(excinfo.value.detail)
