...
#
#   Author: Deb Stacey, Ian McKechnie, Matthew Szurkowski
#
...
# displayTables returns the names of the tables that are in
# the GBADs AWS RDS Postgres database
...
# displayTabInfo returns the column names and datatypes of a
# given table that is in the GBADs AWS RDS Postgres database
...
#    Parameter(s): pointer to database generated by psycopg2,
#       table name, string with comma seperated field/column names
...
# setJoin defines the tables that are to be
# joined and the fields that are in the join
...
def setJoin(table_name1, table_name2, jfield_1, jfield_2):
    jstring = f"""FROM {table_name1} INNER JOIN {table_name2}
    ON {table_name1}.{jfield_1}={table_name2}.{jfield_2}"""
    return jstring


# The query method. Queries the database and returns the answer
#   Parameter(s): Cursor pointer to database generated by psycopg2,
#   table_name name of table, selectstring: Query string, joinstring,
#   order: order by value
#   Returns: The answer to the query
# May 2023 changes:
# selectstring -> sString
# wherestring -> wString
# joinstring -> jString


def query(cursor, table_name, sString, wString, jString, order: Optional[str] = ""):
    if wString == "":
        if jString == "":
            if order != "":
                cursor.execute(
                    "SELECT %s FROM %s " "ORDER BY %s" % (sString, table_name, order)
                )
            else:
                cursor.execute("SELECT %s " "FROM %s" % (sString, table_name))
        else:
            if order != "":
                cursor.execute(
                    "SELECT %s %s " "ORDER BY %s" % (sString, jString, order)
                )
            else:
                cursor.execute("SELECT %s %s " % (sString, jString))
    else:
        if jString == "":
            if order != "":
                cursor.execute(
                    "SELECT %s FROM %s "
                    "WHERE %s ORDER BY %s" % (sString, table_name, wString, order)
                )
            else:
                cursor.execute(
                    "SELECT %s FROM %s " "WHERE %s" % (sString, table_name, wString)
                )
        else:
            if order != "":
                cursor.execute(
                    "SELECT %s %s WHERE %s "
                    "ORDER BY %s" % (sString, jString, wString, order)
                )
            else:
                cursor.execute("SELECT %s %s " "WHERE %s" % (sString, jString, wString))
...
#   Parameter(s): Cursor pointer to database generated by psycopg2,
#   table_name name of table, selectstring: Query string,
#   joinstring, order: order by value
#   Returns: The answer to the query
# May 2023 changes:
# selectstring -> sString
# wherestring -> wString
...
def countQuery(
    cursor, table_name, sString, wString, jString, order: Optional[str] = ""
):
    if wString == "":
        if jString == "":
            if order != "":
        ...
        if jString == "":
            if order != "":
                cursor.execute(
                    "SELECT count(*) "
                    "FROM %s WHERE %s "
                    "ORDER BY %s" % (table_name, wString, order)
                )
            else:
                cursor.execute(
                    "SELECT count(*) " "FROM %s WHERE %s" % (table_name, wString)
                )
        else:
            if order != "":
                cursor.execute(
                    "SELECT %s %s "
                    "WHERE %s ORDER BY %s" % (sString, jString, wString, order)
                )
            else:
                cursor.execute("SELECT %s %s " "WHERE %s" % (sString, jString, wString))
...
# setCountQuery builds a query to be sent to
# the database to retrieve the number of records
...
def setCountQuery(table_name, selectstring, wherestring, joinstring):
    if wherestring == "":
        ...
        if joinstring == "":
            querystring = f"""SELECT count(*)
            FROM {table_name} WHERE {wherestring}"""
        else:
            querystring = f"""SELECT {selectstring} {joinstring}
            WHERE {wherestring}"""
    return querystring
